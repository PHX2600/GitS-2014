...
public class Boar : Enemy
{
	...
	private bool LookForPlayers()
	{
		...
		if (gameObject != null)
		{
			this.state = Boar.State.RunToPlayer;
	...
	{
		this.state = Boar.State.RandomWalk;
	...
	public override void UpdateAI()
	{
		...
		case Boar.State.Idle:
			...
			break;
		case Boar.State.RandomWalk:
			if (!this.LookForPlayers())
			{
				this.UpdateRandomWalk();
			}
			break;
		case Boar.State.RunToPlayer:
			this.UpdateTargetedPlayer();
			break;
		case Boar.State.Attack:
			this.UpdateAttack();
			break;
		}
	}
